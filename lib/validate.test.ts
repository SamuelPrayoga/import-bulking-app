...
describe("validateSubmissionRows", () => {
  ...
  it("marks a fully correct row as valid with recomputed codes", async () => {
    const [result] = await validateSubmissionRows([row({})], baseCtx);
  ...
  it("flags a NIK that isn't 16 digits", async () => {
    const [result] = await validateSubmissionRows([row({ nik: "12345" })], baseCtx);
    expect(result.status).toBe("invalid");
    expect(result.errors).toContain("NIK harus 16 digit angka");
  });

  it("cleans stray non-digit characters out of the NIK before validating it (spaces, dashes, a force-text apostrophe, ...)", async () => {
    const [result] = await validateSubmissionRows([row({ nik: "1811-0101 0190'0001" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.nik).toBe("1811010101900001");
  });

  it("still rejects a NIK that's the wrong length even after cleaning non-digits out", async () => {
    const [result] = await validateSubmissionRows([row({ nik: "1811-0101-9000-1" })], baseCtx);
    expect(result.status).toBe("invalid");
    expect(result.errors).toContain("NIK harus 16 digit angka");
  });

  it("does NOT cross-check NIK digits against the province/city code", async () => {
    ...
    const [result] = await validateSubmissionRows([row({ nik: "9999010101900001" })], baseCtx);
    expect(result.status).toBe("valid");
  });

  it("flags ALL rows sharing a duplicated NIK within the same file", async () => {
    const rows = [row({ rowNumber: 7, nik: "1811010101900001" }), row({ rowNumber: 8, nik: "1811010101900001" })];
    const [first, second] = await validateSubmissionRows(rows, baseCtx);
  ...
  it("warns (but does not invalidate) a NIK flagged as read from a numeric cell at risk of precision loss", async () => {
    const [result] = await validateSubmissionRows(
  ...
  it("does not warn when the NIK is fine (nikNumericRisk false or unset)", async () => {
    const [result] = await validateSubmissionRows([row({})], baseCtx);
    expect(result.warnings).toEqual([]);
  });

  it("does not block a NIK that already exists in an earlier submission under the same name, but still warns so the operator notices the repeat — a legitimate resubmission, not gated here since the destination CMS already handles it", async () => {
    const ctx = {
      ...baseCtx,
      findNikHistory: () => ({ nama: "BUDI SANTOSO", picName: "Ani", timestamp: "2026-08-01" }),
    };
    const [result] = await validateSubmissionRows([row({})], ctx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
    expect(result.warnings.some((w) => w.includes("NIK sama pernah disubmit pada submission sebelumnya"))).toBe(true);
  });

  it("warns (but does not invalidate) a NIK that exists in an earlier submission under a different name", async () => {
    const ctx = {
      ...baseCtx,
  ...
  it("does not validate the agent's own No WA format", async () => {
    const [result] = await validateSubmissionRows([row({ noWa: "not-a-phone-number" })], baseCtx);
    expect(result.status).toBe("valid");
  });

  it("cleans stray non-digit characters out of the agent's No WA", async () => {
    const [result] = await validateSubmissionRows([row({ noWa: "(0812) 3456-7890" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.noWa).toBe("081234567890");
  });

  it("treats a blank No WA as optional, not a validation error", async () => {
    const [result] = await validateSubmissionRows([row({ noWa: "" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
  });

  it("auto-corrects an unrecognized JOB to 'Lainnya' with a warning, instead of blocking the row", async () => {
    // Real case: a village/area name ("GP. GEUCEU KOMPLEK") typed into the JOB column by mistake.
    const [result] = await validateSubmissionRows([row({ job: "GP. GEUCEU KOMPLEK" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
  ...
  it("auto-corrects a blank JOB to 'Lainnya' with a warning too, instead of blocking the row", async () => {
    const [result] = await validateSubmissionRows([row({ job: "" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
    expect(result.job).toBe("Lainnya");
    expect(result.jobId).toBe("10");
  ...
  it("normalizes JOB to Master Data's canonical spelling once resolved, including via an alias (e.g. 'POLRI' -> the actual position 'Bhabinkamtibmas')", async () => {
    const [result] = await validateSubmissionRows([row({ job: "POLRI" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.job).toBe("Bhabinkamtibmas");
    expect(result.jobId).toBe("12");
  });

  it("auto-corrects a Kota Kabupaten that doesn't belong to the file's Provinsi to the PIC's own declared kab/kota, with a warning instead of blocking the row", async () => {
    ...
    const [result] = await validateSubmissionRows([row({ kotaKabupaten: "BOGOR" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
  ...
  it("still blocks the row as invalid when even the PIC's own declared kab/kota can't be resolved", async () => {
    const ctx = { ...baseCtx, declaredKabKota: "TEMPAT TIDAK ADA" };
    const [result] = await validateSubmissionRows([row({ kotaKabupaten: "BOGOR" })], ctx);
    expect(result.status).toBe("invalid");
    expect(result.errors.some((e) => e.includes("tidak sesuai dengan Provinsi"))).toBe(true);
  });

  it("flags required fields that are empty", async () => {
    const [result] = await validateSubmissionRows([row({ nama: "" })], baseCtx);
    expect(result.status).toBe("invalid");
    expect(result.errors).toContain("Nama kosong");
  });

  it("standardizes Nama to uppercase", async () => {
    const [result] = await validateSubmissionRows([row({ nama: "Budi Santoso" })], baseCtx);
    expect(result.nama).toBe("BUDI SANTOSO");
  });

  it("does not flag a row whose Kota/Kabupaten differs from what the PIC declared in the Form — one PIC can manage agents across several kab/kota", async () => {
    const [result] = await validateSubmissionRows([row({ kotaKabupaten: "MESUJI" })], baseCtx);
    expect(result.status).toBe("valid");
    expect(result.errors).toEqual([]);
  });
});
